...
def check_teacher_exists(username, name):
    response = (
        supabase.table("teachers").select("username").eq("username", username).execute()
    )
    name_response = supabase.table("teachers").select("name").eq("name", name).execute()
    return len(response.data) > 0 or len(name_response.data) > 0
